...
void Set_TS_Header(STRUCT_TS *pts, WORD PID, int Adap_field_ctrl, int PayloadStart, bool PCR_Flag)
{
	...
	pts->TS_Output = new (NonPagedPool) BYTE[pts->TS_Length];
...
//#endif
		...
		{		
			...
			pts->TS_Adaptation_Field = new (NonPagedPool) BYTE[Adap_Field_len];
						 ...
						 BYTE *OutputData_Buffer, int & OutputBufSize)
{
	...
	if(PES_NumBytes > MAX_REMAINBYTES)
	{
		...
		{
			// Construct TS header for remian PES.part & Set PCR for last TS packect of PES
			Set_TS_Header(pTS_PES, Elementary_PID, PES_ADAPTATION_FIELD, NON_PAYLOAD_START, PCR_NO_SET);
...
int Construct_PES(STRUCT_PES *ppes, const BYTE *pVideoES, int NumBytes)
{
	...
	if(ppes->Stream_ID == 0xE0)
	{
		...
		{	
			...
			ppes->PES_Header_Fields = new (NonPagedPool) BYTE [ppes->PES_Header_Data_Length];
...
int Combine_PES(STRUCT_PES *ppes)
{
	...
	ppes->PES_Output = new (NonPagedPool) BYTE [Output_Size];
...
void Construct_PAT(STRUCT_PAT *ppat)
{
	...
	ppat->PATable= new (NonPagedPool) BYTE [PAT_Size];
	...
	ppat->Stuffing_Data = new (NonPagedPool) BYTE [Stuffing_length];
...
{
	unsigned char *reCRCInput = new (NonPagedPool) unsigned char [8+Program_Count*4];
...
{
	unsigned char *reCRCInput = new (NonPagedPool) unsigned char [12+Program_Elementary_Count*5];
...
void Combine_PAT(STRUCT_PAT *ppat,int Program_Count,int Stuffing_length)
{
	...
	ppat->PAT_Output = new (NonPagedPool) BYTE [ppat->Output_length];
...
void Construct_PMT(STRUCT_PMT *ppmt)
{
	...
	ppmt->PMTable= new (NonPagedPool) BYTE [PMT_Size];
	...
	ppmt->Stuffing_Data = new (NonPagedPool) BYTE [Stuffing_length];
...
void Combine_PMT(STRUCT_PMT *ppmt,int Program_Element_Count,int Stuffing_length)
{
	...
	ppmt->PMT_Output = new (NonPagedPool) BYTE [ppmt->Output_length];
...
void InitGlobalVar()
{
	...
	g_llPCR_Accumulator = 0;
	g_llPTS_Accumulator = 20;
	g_llDTS_Accumulator = 10;
