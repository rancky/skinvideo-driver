...
{
	// shawn 2011/05/06 modify +++++
	PDEVEXT pDevExt = (PDEVEXT)pUsbDev->m_pDevExtTmp;

	DbgPrint("Device goes into Selective Suspend..., m_bDeviceIdle = %d, DevicePowerState = %d", pDevExt->m_bDeviceIdle, pDevExt->DevicePowerState);

	if (pDevExt->m_bDeviceIdle && pDevExt->DevicePowerState == PowerDeviceUnspecified)
	{
		// Vista cannot accept PowerDeviceD3, that XP does.
		if (pDevExt->pVideoDevice->RunningWindowsVersion() == WindowsXP)
		{
			pUsbDev->SendDeviceSetPower(PowerDeviceD3, TRUE);  //RBK , send device to D3 if XP, avoid hangging in shutdown issue.
			DbgPrint("Camera goes into Selective Suspend D3\n");
		}
		else
		{
			pUsbDev->SendDeviceSetPower(PowerDeviceD2, TRUE);
			DbgPrint("Camera goes into Selective Suspend D2\n");
		}
	}
	// shawn 2011/05/06 modify -----
}
...
NTSTATUS UsbDev::SubmitIdleRequestIrp()
{
	...
	PDEVEXT pDevExt = (PDEVEXT)m_pDevExtTmp;	// shawn 2011/05/09 add
	...
	// shawn 2011/05/09 +++++
	if (InterlockedCompareExchangePointer((PVOID *)&pDevExt->DevicePowerState, (PVOID)PowerDeviceUnspecified, (PVOID)PowerDeviceD0) != (PVOID)PowerDeviceD0)
	{
		DbgPrint("Invalid device power state (%d)\n", pDevExt->DevicePowerState);
		InterlockedExchangePointer((PVOID *)&m_SuspendIrp, NULL);
		IoFreeIrp(Irp);
