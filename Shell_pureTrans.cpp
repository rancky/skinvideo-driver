...
//2010/11/24 06:32下午
void CShell_S420_YUY2_Process(CShell *mp)

{
...

           {
                     ...
                     for (i=0; i<iDecHSize/8; i++)//YYYYUU

                     {

                                pYUY2Start[0] = pStream[j*iSLineStep + (i*6 )];//Y

                                pYUY2Start[1] = pStream[j*iSLineStep + (i*6 +4)];//U

                                pYUY2Start[2] = pStream[j*iSLineStep + (i*6 +1)];//Y

                                pYUY2Start[3] = pStream[(j+1)*iSLineStep + (i*6 +4)];//V

                                pYUY2Start[4] = pStream[j*iSLineStep + (i*6 +2)];//Y

                                pYUY2Start[5] = pStream[j*iSLineStep + (i*6 +5)];//U

                                pYUY2Start[6] = pStream[j*iSLineStep + (i*6 +3)];//Y

                                pYUY2Start[7] = pStream[(j+1)*iSLineStep + (i*6 +5)];//V
                      ...
                      for (i=0; i<iDecHSize/8; i++)//YYYYVV

                     {

                                pYUY2Start[0] = pStream[(j+1)*iSLineStep + (i*6 )];//Y

                                pYUY2Start[1] = pStream[j*iSLineStep + (i*6 +4)];//U

                                pYUY2Start[2] = pStream[(j+1)*iSLineStep + (i*6 +1)];//Y

                                pYUY2Start[3] = pStream[(j+1)*iSLineStep + (i*6 +4)];//V

                                pYUY2Start[4] = pStream[(j+1)*iSLineStep + (i*6 +2)];//Y

                                pYUY2Start[5] = pStream[j*iSLineStep + (i*6 +5)];//U

                                pYUY2Start[6] = pStream[(j+1)*iSLineStep + (i*6 +3)];//Y
	...
	{					
		...
		else if (Input_Mode == diYUYV_S420)
		{
		  DBGU_TRACE("CShell_YUYV_S420_Process\n");
			...
			CShell_S420_YUY2_Process(mp);return;
		}
