...
CShell* CShell_CShell(/*Header *pHeader*/)
{
    ...
    #elif defined(__KERNEL_MODE__)
    {
		...
		// [2008/03/10, Albert]
		if((mp->pLocProcBuf1 = (UInt8 *)ExAllocatePoolWithTag(NonPagedPool, sizeof(UInt8)*(ImgWidList[isCount-1]+16)*(ImgHgtList[isCount-1]*5/3+16)*3/2,'SNX')) == NULL)		
			return NULL;
		if((mp->pLocProcBuf2 = (UInt8 *)ExAllocatePoolWithTag(NonPagedPool, sizeof(UInt8)*(ImgWidList[isCount-1]+16)*(ImgHgtList[isCount-1]*5/3+16)*3/2,'SNX')) == NULL)		
			return NULL;
		// [2008/05/30, Albert]
		if((mp->pStoreMeanBuf = (UInt8 *)ExAllocatePoolWithTag(NonPagedPool, sizeof(UInt8)*(ImgWidList[isCount-1]+16)*(ImgHgtList[isCount-1]*5/3+16)*3/2/16,'SNX')) == NULL)
			return NULL;
...
void CShell_SetProcessInfo(CShell *mp, UInt32 iCapturedLen, UInt8 *pStreamBuf,
    /*UInt8 *pProcBuf,*/ UInt8 *pDisBuf, QTableInfo *pQTableInfo)
{
//    Header *pHeader = mp->pHeader;

    mp->iStreamLen  = iCapturedLen;
    mp->pStreamBuf  = pStreamBuf;
    mp->pStream     = pStreamBuf;
    //mp->pProcBuf    = pProcBuf; // [2008/04/07, Albert]
    mp->pDisBuf     = pDisBuf;
}
//---------------------------------------------------------------------------
//2010/10/21 06:20下午
    ...
    int *piHoSize,  int *piVoSize, int *piSWSubSample, int *piSrcBlkSize)
{
    if (mp->bWaitSync)
    {
        ...
        *piHoSize = mp->iTempHoSize;
        *piVoSize = mp->iTempVoSize;
        *piSWSubSample = mp->iTempSWSubSample;
        *piSrcBlkSize = mp->iTempSrcBlkSize;
    }
    else
    {
        JPEGParam  *pJPEGParam  = CJPEG_GetParam(mp->pJPEG);
        ...
        *piSWSubSample = pScaleParam->iSWSubSample;
        *piSrcBlkSize = pScaleParam->iSrcBlkSize;
    }
}*/
//---------------------------------------------------------------------------
...
void CShell_RawProcess(CShell *mp)
{
    ...
    SInt32 iHSize = pJPEGParam->iOutBufHSize;
    SInt32 iVSize = pJPEGParam->iOutBufVSize;
    SInt32 iYBufSize = iHSize*iVSize;
    SInt32 iCBufSize = iYBufSize/4;

    UInt8 *pRaw = mp->pStream;
    UInt8 *pProcessBuf = mp->pProcBuf;
    UInt8 *pY  = pProcessBuf;
    UInt8 *pCb = pProcessBuf +iYBufSize;
    UInt8 *pCr = pCb +iCBufSize;

    memset(pCb, 128, iCBufSize);
    memset(pCr, 128, iCBufSize);

#if defined(DUMP_STREAM_TO_FILE)
    {
        extern long counter;
        WCHAR fileName[DUMP_STREAM_FILE_MAXPATH];

        swprintf(fileName, DUMP_STREAM_PATH L"yuv%06d.raw", counter);
        KeuWriteFile(fileName, ((PUCHAR)pProcessBuf), (iBufSize*3), NULL, FALSE);
    }
#endif

//    int iStreamHSize = mp->pHeader->CLR_HI_SIZE;
  //  int iStreamVSize = mp->pHeader->CLR_VI_SIZE;
 //   for (int j = 0; j < iStreamVSize; j++)
   //     memcpy(pProcessBuf+iHSize*j, pRaw+iStreamHSize*j, iStreamHSize);
}
//---------------------------------------------------------------------------
...
void CShell_YUV2RGB24(CShell *mp)
{
    ...
    int iYPixStep = iPixStep*2;
    int iCPixStep = iPixStep;
    int iYLineStep = iBufHSize   *iYPixStep;
    int iCLineStep = iBufHSize/2 *iCPixStep;
    ...
    UInt8 *pYFrameEnd   = pYFrameStart + iBufHSize*iVoSize* iPixStep;
    UInt8 *pCFrameStart = mp->pProcBuf +iYBufSize +(iViStart/2)*(iBufHSize/2) +(iHiStart/2);
    UInt8 *pBGRLineStart = mp->pDisBuf;

    int iYMiddleOffset = (iHoSize-iVoSize) *iPixStep /2;
    int iCMiddleOffset = (iHoSize-iVoSize) *iPixStep /2 /2;
    int iBGRMiddleOffset = (iHoSize-iVoSize) *3 /2;
    //if (pShellParam->eRotateState != rsNone)
    ...
    if (iSWSubSample == 0)
    {
        #if defined(__MMX__)
        {
            ...
            {
                iOff0 = 0;
                iOff2 = iYLineSize*iPixStep;
            }
            else if (!bVMirror && bHMirror)
            {
                iOff0 = 4;
                iOff2 = iYLineSize*iPixStep+4;
            }
            else if (bVMirror && !bHMirror)
            {
                iOff0 = iYLineSize*iPixStep;
                iOff2 = 0;
            }
            else if (bVMirror && bHMirror)
            {
                iOff0 = iYLineSize*iPixStep+4;
                iOff2 = 4;
            }
        }
        #else
        {
            ...
            if (!bVMirror && !bHMirror)
            {
                ...
                iOff1 = iPixStep;
                iOff2 = iYLineSize*iPixStep;
                iOff3 = iYLineSize*iPixStep+iPixStep;
            }
            else if (!bVMirror && bHMirror)
            {
                iOff0 = iPixStep;
                iOff1 = 0;
                iOff2 = iYLineSize*iPixStep+iPixStep;
                iOff3 = iYLineSize*iPixStep;
            }
            else if (bVMirror && !bHMirror)
            {
                iOff0 = iYLineSize*iPixStep;
                iOff1 = iYLineSize*iPixStep+iPixStep;
                iOff2 = 0;
                iOff3 = iPixStep;
            }
            else if (bVMirror && bHMirror)
            {
                iOff0 = iYLineSize*iPixStep+iPixStep;
                iOff1 = iYLineSize*iPixStep;
                iOff2 = iPixStep;
                iOff3 = 0;
            }
        }
...
void CShell_YUV2I420(CShell *mp)
{
    ...
    int iYLineStep = iBufHSize   *iYPixStep;
    int iCLineStep = iBufHSize/2 *iCPixStep;
    int iILineStep = (bVMirror)? (-iHoSize)*2: iHoSize*2;
    int iULineStep = (bVMirror)? (-iHoSize)/2: iHoSize/2;
...
void CShell_YUVRotate(CShell *mp)
{
    ...
    int iHCnt = (iHoSize <<iSWSubSample)>>2;
    int iYLineStep = iBufHSize;
    int iCLineStep = iBufHSize/2;
    
    UInt8 *pY = mp->pProcBuf +(2+iViStart)*iBufHSize +iHiStart;
    UInt8 *pCb = mp->pProcBuf +iYBufSize +(2+iViStart/2)*(iBufHSize/2) +(iHiStart/2);
    UInt8 *pCr = mp->pProcBuf +iYBufSize +iCBufSize +(2+iViStart/2)*(iBufHSize/2) +(iHiStart/2);
    ...
    #if defined(__MMX__)
    {
        ...
        Rotate_YH:
            movd            mm0,            [esi+ebx*4];
            movd            mm2,            [edi+ebx*4];
            punpcklbw       mm0,            [esi+ebx*2];
            punpcklbw       mm2,            [edi+ebx*2];
            movq            mm1,            mm0;
            movq            mm3,            mm2;
            movd            mm4,            [esi];
            movd            mm5,            [edi];
            punpcklbw       mm4,            [esi+eax*2];
            punpcklbw       mm5,            [edi+eax*2];
            punpcklwd       mm0,            mm4;
            punpckhwd       mm1,            mm4;
            punpcklwd       mm2,            mm5;
            punpckhwd       mm3,            mm5;
        ...
        Rotate_CbH:
            ...
            movd            mm2,            [edi+ebx*2];
            punpcklbw       mm0,            [esi+ebx];
            punpcklbw       mm2,            [edi+ebx];
            movq            mm1,            mm0;
            movq            mm3,            mm2;
            movd            mm4,            [esi];
            movd            mm5,            [edi];
            punpcklbw       mm4,            [esi+eax];
            punpcklbw       mm5,            [edi+eax];
            punpcklwd       mm0,            mm4;
            punpckhwd       mm1,            mm4;
            punpcklwd       mm2,            mm5;
            punpckhwd       mm3,            mm5;
    ...
    #else
    {
	    ...
	    pY = mp->pProcBuf +(iViStart)*iBufHSize +iHiStart;
	    pCb = mp->pProcBuf +iYBufSize +(iViStart/2)*(iBufHSize/2) +(iHiStart/2);
	    pCr = mp->pProcBuf +iYBufSize +iCBufSize +(iViStart/2)*(iBufHSize/2) +(iHiStart/2);
	
	    pY  += iYMiddleOffset;
	    pCb += iCMiddleOffset;
	    pCr += iCMiddleOffset;
	    
        for (i = 0; i < (iVoSize * iPixStep); ++i)
        {
            si = di = 0;
            for (j = (i + 1); j < (iVoSize * iPixStep); ++j)
            {
                si++;
                di += iYLineStep;
        ...
        for (i = 0; i < ((iVoSize * iPixStep) / 2); ++i)
        {
            si = di = 0;
            for (j = (i + 1); j < ((iVoSize * iPixStep) / 2); ++j)
            {
                si++;
                di += iCLineStep;
...
{
	mp->Param.iFilterThd = iFilterThd;
	mp->Param.iFilterMode = iFilterMode;
}
//---------------------------------------------------------------------------
void CShell_SetEdgeThd(CShell *mp, UInt32 iEdgeThd, UInt32 iEdgeGain)
{
	mp->Param.iEdgeThd= iEdgeThd;
	mp->Param.iEdgeGain = iEdgeGain;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
/*void CShell_FilterProcess(CShell *mp)
{
	SInt32 iFilterThd = mp->Param.iFilterThd;
	if(!iFilterThd)
		return;
	...
	if(((mp->Param.iFilterThd & 0xff)==0) & (iYHSize == 640))
		return;
	if((((mp->Param.iFilterThd>>8) & 0xff)==0) & (iYHSize == 1280))
		return;

	iFilterThd = (iYHSize == 640)?(mp->Param.iFilterThd & 0xff):
				((mp->Param.iFilterThd>>8) & 0xff);
	
	int i,j,P0,P1;
	UInt8 *pLineBuf1 = mp->pTempBuf;
	UInt8 *pLineBuf2 = mp->pTempBuf + iYHSize;
    UInt8 *pY  = mp->pProcBuf + iYVStart*iYBufHSize +iYHStart;
	
	memcpy(pLineBuf2,pY+iYBufHSize,iYHSize);
	for(j=1;j<iYVSize-1;j+=2)
	{
		for(i=1;i<iYHSize-1;i++)
		{
			P0 = (pY[j*iYBufHSize+i]<<2) + pY[(j-1)*iYBufHSize+i-1] + pY[(j-1)*iYBufHSize+i+1] + pY[(j+1)*iYBufHSize+i-1] + pY[(j+1)*iYBufHSize+i+1];
			P1 = (pY[j*iYBufHSize+i-1]+pY[j*iYBufHSize+i+1]+pY[(j-1)*iYBufHSize+i]+pY[(j+1)*iYBufHSize+i])<<1;
			pLineBuf1[i] = (abs(P0-P1) < iFilterThd)?(P0 + P1)>>4:pY[j*iYBufHSize+i];
		}
		memcpy(pY+(j-1)*iYBufHSize+1,pLineBuf2+1,iYHSize-2);
		
		for(i=1;i<iYHSize-1;i++)
		{
			P0 = (pY[(j+1)*iYBufHSize+i]<<2) + pY[j*iYBufHSize+i-1] + pY[j*iYBufHSize+i+1] + pY[(j+2)*iYBufHSize+i-1] + pY[(j+2)*iYBufHSize+i+1];
			P1 = (pY[(j+1)*iYBufHSize+i-1]+pY[(j+1)*iYBufHSize+i+1]+pY[j*iYBufHSize+i]+pY[(j+2)*iYBufHSize+i])<<1;
			pLineBuf2[i] = (abs(P0-P1) < iFilterThd)?(P0 + P1)>>4:*(pY+(j+1)*iYBufHSize+i);
		}
		memcpy(pY+j*iYBufHSize+1,pLineBuf1+1,iYHSize-2);
	}
}*/
//---------------------------------------------------------------------------
/*void CShell_EdgeProcess(CShell *mp)
{
	UInt32 iFilterThd = mp->Param.iFilterThd & 0xff;
	UInt32 iEdgeThd = mp->Param.iEdgeThd;
	...
	int i,j,P0,P1;
	UInt32 iHP;
	SInt16 *pLineBuf1 = (SInt16 *)mp->pTempBuf;
	SInt16 *pLineBuf2 = (SInt16 *)mp->pTempBuf + iYHSize;
	UInt8 *pY  = mp->pProcBuf + iYVStart*iYBufHSize +iYHStart;
	
	memset(pLineBuf2,0,sizeof(SInt16)*iYHSize);

	for(j=2;j<iYVSize-4;j+=2)
	{
		for(i=2;i<iYHSize-4;i++)
		{
			P0 = (pY[j*iYBufHSize+i]<<3);
			P1 = (pY[j*iYBufHSize+i-2]+pY[j*iYBufHSize+i+2]+pY[(j-1)*iYBufHSize+i]+pY[(j+1)*iYBufHSize+i]
				+pY[(j-1)*iYBufHSize+i-2]+pY[(j-1)*iYBufHSize+i+2]+pY[(j+1)*iYBufHSize+i-2]+pY[(j+1)*iYBufHSize+i+2]);
			iHP = abs(P0-P1);
			if(iHP < iFilterThd)
				pLineBuf1[i] = ((P0+P1)>>4)-pY[j*iYBufHSize+i];
			else if(iHP > iEdgeThd)
			{
				pLineBuf1[i] = (SInt16)((iHP-iEdgeThd)*mp->Param.iEdgeGain>>14);
				if(P0 < P1)
					pLineBuf1[i] = -pLineBuf1[i];
			}
			else
				pLineBuf1[i] = 0;
		}
		for(i=2;i<iYHSize-4;i++)
		{
			int Y = pY[(j-1)*iYBufHSize+i]+pLineBuf2[i];
			if(Y>255)
				pY[(j-1)*iYBufHSize+i] = 255;
			else if(Y<0)
				pY[(j-1)*iYBufHSize+i] = 0;
			else
				pY[(j-1)*iYBufHSize+i] = Y;
		}
		
		for(i=2;i<iYHSize-4;i++)
		{
			P0 = (pY[(j+1)*iYBufHSize+i]<<3);
			P1 = (pY[(j+1)*iYBufHSize+i-2]+pY[(j+1)*iYBufHSize+i+2]+pY[j*iYBufHSize+i]+pY[(j+2)*iYBufHSize+i]
				+pY[j*iYBufHSize+i-2]+pY[j*iYBufHSize+i+2]+pY[(j+2)*iYBufHSize+i-2]+pY[(j+2)*iYBufHSize+i+2]);
			iHP = abs(P0-P1);
			if(iHP < iFilterThd)
				pLineBuf2[i] = ((P0+P1)>>4)-pY[(j+1)*iYBufHSize+i];
			else if(iHP > iEdgeThd)
			{
				pLineBuf2[i] = (SInt16)((iHP-iEdgeThd)*mp->Param.iEdgeGain>>14);
				if(P0 < P1)
					pLineBuf2[i] = -pLineBuf2[i];
			}
			else
				pLineBuf2[i] = 0;
		}
		for(i=2;i<iYHSize-4;i++)
		{
			int Y = pY[j*iYBufHSize+i]+pLineBuf1[i];
			if(Y>255)
				pY[j*iYBufHSize+i] = 255;
			else if(Y<0)
				pY[j*iYBufHSize+i] = 0;
			else
				pY[j*iYBufHSize+i] = Y;
...
//---------------------------------------------------------------------------
/*
void CShell_DeNoiseProcess(CShell *mp)
{
	JPEGParam  *pJPEGParam  = CShell_GetJPEGParam(mp);
//	Header *pHeader = mp->pHeader;
	...
	SInt32 iFilterThd = mp->Param.iFilterThd & 0xff;
	SInt32 iEdgeThd = mp->Param.iEdgeThd;
	SInt32 iThd = mp->Param.iEdgeGain & 0x1f;
	
	int i,j;
	int P0,P1,P2,P3,P4;
	UInt8 *pTempBuf = mp->pTempBuf;
	UInt8 *pY = mp->pProcBuf + iYVStart*iYBufHSize +iYHStart;

	UInt32 iHeaderYSum = 0;//pHeader->AE_YSUM0 + pHeader->AE_YSUM1 + pHeader->AE_YSUM2 + 
					//	pHeader->AE_YSUM3 + pHeader->AE_YSUM4 + pHeader->AE_YSUM5 + 
					//	pHeader->AE_YSUM6 + pHeader->AE_YSUM7 + pHeader->AE_YSUM8;
	
	if(iHeaderYSum != 0)
	{
		int iYSumDiff = iHeaderYSum - mp->Report.iHeaderYSum;
		double dYSumDiffRatio = (double)(abs(iYSumDiff))/iHeaderYSum;
		if(dYSumDiffRatio > 0.02)
		{
			iThd = (UInt32)(iThd/(dYSumDiffRatio*50));
		}
	}

	mp->Report.iHeaderYSum = iHeaderYSum;

	if(iFilterThd)
	{
		for(j=1;j<iYVSize-1;j++)
		{
			for(i=1;i<iYHSize-1;i++)
			{
				P0 = (pY[j*iYBufHSize+i]);
				P1 = (pY[j*iYBufHSize+i+1]);
				P2 = (pY[j*iYBufHSize+i-1]);
				
				if(abs(P1 + P2-(P0<<1)) < iFilterThd)
				{
					P3 = (pY[(j+1)*iYBufHSize+i]);
					P4 = (pY[(j-1)*iYBufHSize+i]);
					pY[j*iYBufHSize+i] = ((P0<<2)+P1+P2+P3+P4)>>3;
				}						
				else if(P0 - P1 > iEdgeThd)
				{
					P3 = (pY[(j+1)*iYBufHSize+i]);
					P4 = (pY[(j-1)*iYBufHSize+i]);
					if(	(P0 - P2 > iEdgeThd) &
						...
						(P0 - P4 > iEdgeThd) )
					{
						pY[j*iYBufHSize+i] = max(P1,max(P2,max(P3,P4)));
					}
				}
				else if(P1 - P0 > iEdgeThd)
				{
					P3 = (pY[(j+1)*iYBufHSize+i]);
					P4 = (pY[(j-1)*iYBufHSize+i]);
					if(	(P2 - P0 > iEdgeThd) &
						...
						(P4 - P0 > iEdgeThd) )
					{
						pY[j*iYBufHSize+i] = min(P1,min(P2,min(P3,P4)));
					}
				}
				
				if(	abs(pY[j*iYBufHSize+i+1]-pTempBuf[j*iYHSize+i+1])+
					abs(pY[(j+1)*iYBufHSize+i-1]-pTempBuf[(j+1)*iYHSize+i-1])+
					abs(pY[(j+1)*iYBufHSize+i]-pTempBuf[(j+1)*iYHSize+i])+
					abs(pY[(j+1)*iYBufHSize+i+1]-pTempBuf[(j+1)*iYHSize+i+1]) < iThd)
				{
					pY[j*iYBufHSize+i] = (pY[j*iYBufHSize+i]+pTempBuf[j*iYHSize+i])>>1;
				}
				
				pTempBuf[j*iYHSize+i] = pY[j*iYBufHSize+i];
			}
		}
	}
	else
...
//---------------------------------------------------------------------------
/*
void CShell_NonLinearUVGain(CShell *mp)
{
	if(!mp->Param.bNLUVGainEnable)
		return;
	...
	UInt8 *pCb = mp->Param.pProcBuf  + iYBufHSize*iYBufVSize;
	UInt8 *pCr = pCb + iCBufHSize*iCBufVSize;

	SInt32 iPUGain = mp->Param.iPUGain;
	SInt32 iPVGain = mp->Param.iPVGain;
	SInt32 iCb,iCr;
	...
	for(i=0;i<iCVSize;i++)
	{
		for(j=0;j<iCHSize;j++)
		{
			if(pCb[i*iCBufHSize+j] > 128)
			{
				iCb = pCb[i*iCBufHSize+j];
				iCb = 128 + (((iCb-128)*iPUGain)>>10);
				pCb[i*iCBufHSize+j] = (iCb < 0)?0:iCb;
			}
		}
	}
	for(i=0;i<iCVSize;i++)
	{
		for(j=0;j<iCHSize;j++)
		{
			if(pCr[i*iCBufHSize+j] > 128)
			{
				iCr = pCr[i*iCBufHSize+j];
				iCr = 128 + (((iCr-128)*iPVGain)>>10);
				pCr[i*iCBufHSize+j] = (iCr < 0)?0:iCr;
			}
		}
	}
}*/
...
void CShell_YUYV_S420_Process(CShell *mp)
{
	...
	for (j=0; j<iDecVSize; j+=2)
	{
		for (i=0, m=0; i<(iDecHSize*3)/4; i+=6, m+=4)
		{
			for (n=0; n<2; n++)
			{
				int jj = j+n;
				pYLine[jj*iYLineStep + (m  )] = pSLine[jj*iSLineStep + (i  )];
				pYLine[jj*iYLineStep + (m+1)] = pSLine[jj*iSLineStep + (i+1)];
				pYLine[jj*iYLineStep + (m+2)] = pSLine[jj*iSLineStep + (i+2)];
				pYLine[jj*iYLineStep + (m+3)] = pSLine[jj*iSLineStep + (i+3)];

				if ( ( (j+n)&1 ) == 0 )
				{
					pCb[(jj/2)*iCLineStep + ((m/2)  )] = pSLine[jj*iSLineStep + (i+4)];
					pCb[(jj/2)*iCLineStep + ((m/2)+1)] = pSLine[jj*iSLineStep + (i+5)];
				}
				else
				{
					pCr[(jj/2)*iCLineStep + ((m/2)  )] = pSLine[jj*iSLineStep + (i+4)];
					pCr[(jj/2)*iCLineStep + ((m/2)+1)] = pSLine[jj*iSLineStep + (i+5)];
				}													 
			}
		}			
	}
